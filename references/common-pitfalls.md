...
| 只跑 8d 变更关键词对账，以为跨文件语义打架查过了 | 8d 关键词表 = 本轮改造的旧词（grep 仍是全目录），历史轮次留下的旧口径不在本轮关键词集里，deep 文件永不命中（2026-09-16 实测：某 skill 四轮验收全绿 vs 同日全量深读抓 3 处判据打架） | 全量档跑 8g：跨文件重复判据逐对双向核方向，不依赖本轮变更关键词 |
...
| 散文落点路径核不到（8d 同族） | 正文句子里的「产物存到 `<某路径>`」「归档到 X 目录」不是引用语法，第 2 步的孤儿/断链检测天然扫不到 | 把路径拿到真实磁盘 `ls` 核对存在性；同动作在姊妹 skill 有另一种写法时先对比口径，两边矛盾必有一假（2026-09 实测沉淀） |
