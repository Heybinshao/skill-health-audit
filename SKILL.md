...
先跑 `python3 <本skill目录>/scripts/criteria_overlap.py <skill_dir>`——自动提取**跨文件重复 token**（行内代码 token、数值阈值、计数点名、步骤/闸门号引用四类）并列出出现在 2+ 文件的候选对；脚本只出线索，误报率不低（示例词、泛用词），定性永远是人。对照实测（2026-09-16）：对人工判定无打架的 path-simulation 出 6 候选全假阳性（`NUM:3条`、`REF:步骤` 类同源泛词），对某多文件 skill 出 41 候选含真打架线索——**候选信噪比低是设计内**：FP 是看一眼即判一致的无害候选，本步真正要防的是漏报，勿因 FP 弃用或收紧规则。无脚本环境（第三方安装缺 scripts/）按下列七形态手工枚举——**前六形态各来自 2026-09-16 四轮验收实战，每形态至少抓到一处真打架；第七形态来自 2026-09-22 本 skill 自身独立验收（P1/P2 两案）**：
...
**触发档位**：全量验收（scope=全量）、大版本/架构改造后、或修复引入「新口径替代旧口径」时必做（双版本并存迹象 = 触发信号）；**含多参考文件时增量轮亦必走**（对象可缩为「本轮新口径 vs 全目录存量判据」配对，成本可控）；确要跳过（仅限单参考文件以下的对象），须在报告「覆盖」行显式标注「8g 未跑」。**本节为 scope 判据唯一权威；skill-acceptance Phase 0 条 3 在其上加编排侧从严信号，不另立判据**。**编排侧加严信号已于 2026-09-22 撤销**：曾由 acceptance 加「references ≥5 个自动升全量」，实测反噬（某 16 个 references 的 skill → 每轮自动重跑全量、30+ 调用 / 5-25 分钟）；**references 数量不是全量触发信号**，只决定增量口径下的配对范围（本轮新口径 vs 全存量判据）。

**获取判据对的方式（2026-09-22 提速，防「全量 = 通读全部」）**：形态 1/2/4/5 → 先跑 `criteria_overlap.py` 出候选，**只读命中涉及的文件**（某 16 个 references 的 skill 实测只需读 3-4 个）；形态 3/6/7（范围义务 / 外部权威 / 能力行为声称）脚本提取不到 → 用 **lure 正则**定向抓：3=`必须|唯一|一律|禁止|不得|所有|每条|凡`、6=`源码|第 *[0-9]+ *行|官方|\.py:[0-9]+`、7=`支持|能查|可查|会自动|会报|退出码|可传|接受`。**收工锚是「每条判据对有结论」，不是「每个文件都逐字读过」**——逐字通读只在两个场合恢复：大版本手术、发布前的全量档（2026-09-22 实测：全量通读 5.4 万字符的对象 = 30+ 调用）。
...
- **改过检查器脚本本身 → 验证三件套（只跑正例等于没验）**：①**正例**（原本健康的对象仍绿）；②**负例夹具**（临时目录造一个：超限 description + 死 `triggers:` + 孤儿 references + 一个全库都不存在的断裂引用 + 与库内某 skill 同名的 `name`，跑完应全部红旗、退出码 1）；③**回归**（本轮改动涉及的正则/口径，拿原先全绿的对象复跑，确认没从「该红」变「哑」或「该绿变红」）。检查器改坏是**静默故障**——判据变松、发红变哑都不会自己冒出来：正例只证明「没把绿的整红」，证明不了「红的还能红」。夹具用完可删，重建成本就是那五行 frontmatter。
...
`<你的 skill 安装目录>/scripts/preflight_check.py <skill_dir>` —— **开场预检一次跑完**：元信息（description 长度与超限、死 `triggers:` 字段、python len 字符数、references 估读量）、引用完整性（孤儿/断裂，占位名自动排除）、第 2b 步同名副本遮蔽、第 2c 步指路语（**强信号=真指路语逐条读 / 弱信号=仅编号枚举抽样**）、8g 下一步线索（`criteria_overlap` 命令 + 形态 3/6/7 的 lure 正则）。**替代十余条散装命令的往返与输出回灌**（2026-09-22 立）；退出码 0 = 无机械问题 / 1 = 有 / 2 = 用法错误。
