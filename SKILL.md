...
**获取判据对的方式（2026-09-22 提速，防「全量 = 通读全部」）**：形态 1/2/4/5 → 先跑 `criteria_overlap.py` 出候选，**只读命中涉及的文件**（某多文件 skill 16 个 references 实测只需读 3-4 个）；形态 3/6/7（范围义务 / 外部权威 / 能力行为声称）脚本提取不到 → 用 **lure 正则**定向抓：3=`必须|唯一|一律|禁止|不得|所有|每条|凡`、6=`源码|第 *[0-9]+ *行|官方|\.py:[0-9]+`、7=`支持|能查|可查|会自动|会报|退出码|可传|接受`。**收工锚是「每条判据对有结论」，不是「每个文件都逐字读过」**——逐字通读只在两个场合恢复：大版本手术、发布前的全量档（2026-09-22 实测：全量通读 某多文件 skill 5.4 万字符 = 30+ 调用）。
...
`<你的 skill 安装目录>/scripts/selftest.py` —— **两个脚本的自测**（无参数，夹具建在临时目录、跑完即删）：19 条断言覆盖五组真实踩坑——良构 skill 双绿且不被误报灯、病构 skill 双红并点名孤儿/断裂/重复二级标题/代码块未配对/description 越界、含点文件名不得被截断成 `08.md`、跨 skill 引用判 ℹ️ 非问题、`--scan-all` 抓到超限项。**改完任一脚本先跑它**；退出码 0 = 全过。发布版同捆，别人 clone 后也能自证（2026-09-22 立：此前每次改动都靠临时手搓夹具验证，验完即弃，本轮写它时立刻抓出两条假绿——断言词出现在夹具 description 里、「被引用的文件」不算孤儿）。
